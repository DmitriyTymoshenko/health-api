...
/**
 * #1492 (stage H of #1485): extracted from routes/supplement_stack.js's
 * inline "most-recently-started cycle per supplement_id wins" loop (#1489,
 * E6) so lib/stock-notify.js can reuse it by name instead of re-declaring the
 * same fold — same rationale as that file's own comment: a `restart` creates
 * a NEW cycle doc rather than mutating the old one, so picking by
 * `start_date` (not filtering on `status`) is the only way to always land on
 * the CURRENT cycle.
 * @param {Array<object>} cycles supplement_cycles docs
 * @returns {Map<number, object>} supplement_id -> most recent cycle doc
 */
function mostRecentCycleBySupplementId(cycles) {
  const bySupplementId = new Map()
  for (const cycle of cycles) {
    const existing = bySupplementId.get(cycle.supplement_id)
    if (!existing || cycle.start_date > existing.start_date) {
      bySupplementId.set(cycle.supplement_id, cycle)
    }
  }
  return bySupplementId
}

module.exports = { computeStock, mostRecentCycleBySupplementId }
