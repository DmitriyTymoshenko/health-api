...
function buildPrompt(supplementLabel, evidenceText) {
  return `You are assisting a supplement-tracking app. You are given (1) retrieved nutrition-course excerpts (Ukrainian, distilled from Oleksandr Koliada's course) and (2) the name of a dietary supplement.

Determine whether this supplement is safe to take CONTINUOUSLY (no scheduled breaks) or whether it should be taken in CYCLES (a period of daily use followed by a rest/pause period). If the retrieved excerpts discuss this supplement or its active ingredient directly, prefer a Koliada verdict and cite it precisely (a real "Lesson N" or "урок N" reference AND a verbatim quote copied from the excerpts below — do not paraphrase, and do not cite Koliada text outside these excerpts). Use an external source (examine.com, cochranelibrary.com, ods.od.nih.gov, or pubmed.ncbi.nlm.nih.gov) only when the excerpts do not cover it, or set verdict.kind to "not_covered" if you are not confident.
...
=== RETRIEVED KOLIADA EXCERPTS START ===
${evidenceText}
=== RETRIEVED KOLIADA EXCERPTS END ===`
}
...
function computeFieldsToWrite(existing, llmResult, groundedSource, purchaseUrl) {
  ...
  } else if (doc.continuous === true && isValidCycleObject(doc.cycle)) {
    // REPAIR path (mirror image of `hasPreexistingCycle` above, QA FAIL
    // round 1 on #1487, Max/codex 22.09 13:24 + Apex triage 13:35): the
    // document is ALREADY sitting in the exact invalid state D4 forbids —
    // `continuous:true` alongside a real, non-null `cycle` object. Before
    // this branch existed, such a doc fell through to the "patch missing
    // cycle.source" branch below (if `source` was undefined) or matched NO
    // branch at all (if `source` was already set) — either way `continuous`
    // was never corrected, and once `cycle.source` got backfilled the doc
    // looked "complete" forever with `needsFill()` never re-offering it for
    // repair. Same precedent as `hasPreexistingCycle`: a real, non-null
    // cycle object is stronger evidence than a `continuous:true` flag — the
    // cycle wins outright, `continuous` is corrected to `false`, and
    // `duration_weeks`/`pause_weeks` are never touched/overwritten, only the
    // missing `cycle.source` sub-field (if any) is patched.
    set.continuous = false
    if (doc.cycle.source === undefined) set['cycle.source'] = groundedSource
