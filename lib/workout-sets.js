 ...
 *
 * #1474: a bodyweight set can carry a REAL positive `weight_kg` too — the owner's own
 * bodyweight, autofilled from `weight_log` by `lib/bodyweight-fill.js` so volume/trend
 * aggregates (#1410/#1417) see a real number instead of treating the set as 0kg. That
 * autofill must NOT flip a session from bodyweight to weighted for progression/ranking
 * purposes (`pickBestSet` below, `lib/exercise-progression.js::selectWorkingSets`,
 * `lib/exercise-trends.js`) — so a set tagged `weight_source: 'bodyweight'` is excluded
 * here regardless of its `weight_kg` value. This is the ONLY place that exclusion is
 * expressed; do not duplicate it at a call site.
 *
 * @param {Array<{weight_kg?: number, weight_source?: string}>} sets
 * @returns {boolean} true if at least one set has weight_kg > 0 from a REAL (non-bodyweight) source
 */
function hasPositiveWeight(sets) {
  return (sets || []).some(s => (s.weight_kg || 0) > 0 && s.weight_source !== 'bodyweight')
}
