...
//     (a) `ref` matches /lesson\s*\d+/i or /урок\s*\d+/i, AND
//     (b) `quote` (after whitespace normalization) is a literal substring of
//         the corpus text (also whitespace-normalized) — a fabricated quote
//         or a real quote that just paraphrases the corpus both fail this,
//         by design: paraphrase-matching is exactly the class of "looks
//         grounded but isn't" this validator exists to catch.
...
const LESSON_REF_RE = /(?:lesson|урок)\s*\d+/i
...
function validateVerdict(verdict, corpusText) {
  ...
  if (verdict.by === 'koliada') {
    ...
    const normalizedCorpus = normalizeWhitespace(corpusText)
    const quoteOk = quote.length > 0 && normalizedCorpus.includes(quote)
    if (refOk && quoteOk) {
...
module.exports = { validateVerdict, isAllowlistedUrl, EXTERNAL_ALLOWLIST_DOMAINS, LESSON_REF_RE }
