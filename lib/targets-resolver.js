...
// #1504 — WHOOP-forecast-aware kcal basis, the layer ABOVE #1099's day-type-average
// (which stays the fallback inside this function — see lib/whoop-forecast-kcal.js).
// THE call site for `kcal` below; do not call resolveDayTypeAwareKcalBasis directly
// from a new consumer (BASE RULE — one metric, one definition).
const { resolveKcalBasisWithForecast } = require('./whoop-forecast-kcal')
...
async function resolveDayTargets(db, date) {
  ...
  // #1504 — WHOOP-forecast-aware basis on top of #1099's day-type analog on top of
  // the #1295 stable basis. `cycle: whoopCycle` reuses the fetch above (Promise.all)
  // instead of a second `findOne({date})` on whoop_cycles.
  const { kcal, basis } = await resolveKcalBasisWithForecast(db, p, date, { cycle: whoopCycle })
  ...
  return {
    date,
    // #1504 — 'day_type_avg' | 'whoop_forecast'. Every consumer of `kcal` below
    // MUST surface this alongside it (BASE RULE) — see lib/whoop-forecast-kcal.js.
    basis,
