...
/**
 * #1408: mirror of exercisesNeedingUnit above, for muscle_group instead of weight_unit.
 * Names of exercises in a session whose exercises_library doc has muscle_group == null
 * (or has no resolved doc at all — treated the same as "unknown", never guessed).
 * Surfaced on the session doc as `needs_muscle_group_clarification` so an unclassified
 * exercise is an explicit, queryable state instead of silently landing in the
 * dashboard's "⚡ Інше" bucket with no trace of why (#1408 triage — 4/36 library
 * entries had muscle_group:null with zero way to fix it after auto-create).
 * @param {Array<{name:string}>} exercises
 * @param {Map<string, {muscle_group?: (string|null)}|undefined>} libByName exercise
 *   name -> its exercises_library doc (or absent if no doc was resolved for that name)
 * @returns {string[]}
 */
function exercisesNeedingMuscleGroup(exercises, libByName) {
  return (exercises || [])
    .filter(ex => {
      const libDoc = libByName.get(ex.name)
      return !libDoc || libDoc.muscle_group == null
    })
    .map(ex => ex.name)
}

/**
 * #1318 KROK 2 (health-api half, owner comment #7454 — "weight_kg дораховується
 * постфактум, ДОПОВНЮЮЧИ наявний запис"): once the owner answers the once-per-exercise
 * unit question, recompute weight_kg for every ALREADY-WRITTEN set of that exercise
 * that was left unresolved (weight_input present, weight_kg still null). Resolved sets
 * are left untouched — this only fills gaps, it never overwrites a value that already
 * has a unit (matches toWeightKg's own "converted ONCE at write time" contract).
 * @param {Array<object>} exercises one session doc's exercises[] array
 * @param {string} exerciseName
 * @param {'kg'|'lb'} weightUnit
 * @returns {{exercises: Array<object>, changed: boolean}} changed=false means this
 *   session doc had nothing to backfill for this exercise — caller should skip the write
 */
function backfillWeightUnitInSession(exercises, exerciseName, weightUnit) {
  let changed = false
  const updated = (exercises || []).map(ex => {
    if (ex.name !== exerciseName) return ex
    const sets = (ex.sets || []).map(s => {
      if (s.weight_input == null || s.weight_kg != null) return s
      changed = true
      return { ...s, weight_unit: weightUnit, weight_kg: toWeightKg(s.weight_input, weightUnit) }
    })
    return changed ? { ...ex, sets } : ex
  })
  return { exercises: updated, changed }
}
