 ...
 * doc or null. `evidenceText` = retrieval-first Koliada snippets formatted
 * by lib/koliada-retrieve.js (not the full corpus).
 */
function buildRecsPrompt({ activeStack, knowledgeByCatalogId, latestLabs, whoopLatest, evidenceText, corpusText }) {
  ...
  const snippets = evidenceText ?? corpusText ?? ''

  return `You are a supplement advisor for a health-tracking app. Given the person's CURRENT supplement stack, recent lab results, recent WHOOP recovery/sleep/strain data, and retrieved nutrition-course excerpts (distilled from Oleksandr Koliada's course, Ukrainian), produce a JSON list of supplement recommendations.
...
Return at most 3 items total. Prioritize the highest-confidence, most actionable items; it is better to return [] than to fill space.

For EVERY item, cite your evidence: if the retrieved Koliada excerpts below discuss it directly, you MUST prefer a Koliada verdict over an external verdict: cite the snippet's real "Lesson N"/"урок N"/lesson-range reference AND a verbatim quote copied from those excerpts — never paraphrase, never invent a citation, and never cite Koliada text that is not present in the retrieved excerpts below. Use an external source (examine.com, cochranelibrary.com, ods.od.nih.gov, or pubmed.ncbi.nlm.nih.gov) only when the retrieved excerpts do not cover that supplement/ingredient. If you are not confident either way, set verdict.kind to "not_covered" — this is honest and expected, not a failure.

Do NOT output items derived from a LAB marker marked [STALE]. The app already surfaces stale-lab warnings separately, so stale labs must be omitted from the JSON items array. Use lab-derived items only when the marker is not stale (age_days <= 30).
