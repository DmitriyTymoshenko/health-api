...
// ---------------------------------------------------------------------------
// #1408: the FULL target-enum for exercises_library.muscle_group, used by
// routes/workouts.js PATCH /exercises/:name/muscle-group to validate an owner-supplied
// answer. NOT the same thing as MUSCLE_SOURCE_TO_GROUP above (Dictionary B, source ->
// enum for the free-exercise-db import) — that map's VALUES are a 9-of-10 subset of
// this list (it has no 'other' target because the catalog import always resolves a
// concrete group). Before this export, the only complete 10-value list lived in the
// FRONTEND (health-dashboard/src/pages/Workouts.jsx:34 MUSCLE_ORDER) — the backend had
// no enum to validate an owner's answer against at all. This is now the single source
// for the backend; extend here first if a group is ever added.
// ---------------------------------------------------------------------------
const MUSCLE_GROUPS = ['chest', 'back', 'shoulders', 'legs', 'biceps', 'triceps', 'core', 'forearms', 'neck', 'other']
...
/**
 * #1334 (Ф3/#1331, ЕТАП 1б) — composes Dictionary A (source-keyed display labels)
 * with Dictionary B (source -> OUR enum) into a map keyed by OUR enum, which is the
 * shape the frontend equipment filter/chips actually consume. Reading
 * `DISPLAY_LABELS_UA.equipment` directly by our enum value is WRONG — Dictionary A is
 * keyed by the SOURCE term ('body only', 'kettlebells', 'e-z curl bar', ...) and only
 * 5 of 12 keys are byte-identical to our enum (machine/dumbbell/cable/barbell/other);
 * the other 7 would silently resolve to `undefined` — an empty chip, no error (#1334
 * comment #7581 §2, the exact defect this function exists to close).
 *
 * The composition is exactly 1:1 over `EQUIPMENT_SOURCE_TO_OURS`'s non-null range: 12
 * source keys map onto 12 distinct our-enum values (no two source keys collapse onto
 * the same our-enum value), so every value in that range gets EXACTLY one label —
 * this is the invariant `src/tests/exercise_dictionaries.test.ts` checks by iterating
 * the range, not by spot-checking a few values.
 *
 * Throws if Dictionary A is ever missing a label for a Dictionary B source key — a
 * silent `undefined` here would ship as an empty chip in production, exactly the bug
 * class this composition replaces.
 *
 * @returns {Record<string,string>} our equipment enum value -> UA label. Labels are
 *   stored LOWERCASE (Dictionary A convention) — capitalizing for display is left to
 *   the consuming UI component (#1334 comment #7581 §4), never baked in here.
 */
function getEquipmentLabelsByOurEnum() {
  const labels = {}
  for (const [sourceKey, oursKey] of Object.entries(EQUIPMENT_SOURCE_TO_OURS)) {
    if (sourceKey === NULL_KEY || oursKey === null) continue // source "doesn't know" — not a labelable value
    const label = DISPLAY_LABELS_UA.equipment[sourceKey]
    if (!label) {
      throw new Error(`exercise-dictionaries: Dictionary A has no label for source equipment ${JSON.stringify(sourceKey)} (our enum ${JSON.stringify(oursKey)}) — composition would silently emit an empty chip`)
    }
    labels[oursKey] = label
  }
  return labels
}
