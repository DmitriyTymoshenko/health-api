...
/**
 * #1487 (stage B of #1485, design D4): enforce the `supplement_knowledge`
 * continuous/cycle invariant BEFORE it ever reaches Mongo — a document that
 * claims BOTH "safe to take continuously" AND carries a cycle schedule (or
 * neither) is a silent contradiction no reader downstream could resolve.
 * Only validates when the request BODY actually touches these fields — a
 * `PUT /knowledge/:id` that updates e.g. only `purchase_url` must not be
 * rejected for an invariant it never mentioned. Returns an error string or
 * `null`; the pure function is exported for unit tests, the Express wrapper
 * below is what routes actually use.
 */
function validateKnowledgeCycleInvariant(body) {
  if (!body || typeof body !== 'object') return null
  const hasContinuous = body.continuous !== undefined
  const hasCycle = body.cycle !== undefined
  if (!hasContinuous && !hasCycle) return null

  const continuous = body.continuous
  const cycle = body.cycle

  if (continuous === true && cycle !== null && cycle !== undefined) {
    return 'continuous:true requires cycle:null'
  }
  if (continuous === false) {
    if (cycle === null || cycle === undefined) {
      return 'continuous:false requires a cycle object with duration_weeks >= 1'
    }
    if (typeof cycle !== 'object' || !Number.isInteger(cycle.duration_weeks) || cycle.duration_weeks < 1) {
      return 'cycle.duration_weeks must be an integer >= 1'
    }
  }
  return null
}

function validateSupplementKnowledgeCycle(req, res, next) {
  const error = validateKnowledgeCycleInvariant(req.body)
  if (error) {
...
module.exports = {
  ...
  validateKnowledgeCycleInvariant,
  validateSupplementKnowledgeCycle,
}
