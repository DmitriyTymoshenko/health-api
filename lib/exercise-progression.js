...
// ---------------------------------------------------------------------------
// TOP_OF_RANGE_RULE — owner decision 2026-09-09 (Дмитро, «роби поки mean», task #1291
// comment #7422, verified in agent_logs id 35769 18:15:40Z, telegram:phil). The owner
// was shown all 4 variants scored against his own descending-reps bench-press log
// (8/7/6 at an 8-12 range) plus Phil's recommendation of 'first' — and chose 'mean'
// AGAINST that recommendation, deliberately. Do NOT "fix" this back to 'first' on the
// assumption it's an unreviewed default — it is not.
// PROVISIONAL for the current weight-calibration stage: word "поки" ("for now") is part
// of the decision, not filler — only 3 sessions exist in the live `workouts` collection
// and there is no live entry point into this engine yet (tracked separately, #1314).
// Exactly ONE production read site (the `variant` default parameter of
// evaluateProgression() below) — do not add a second.
// Comparison rule for 'mean' (Apex, #1291 comment, matches the code below): the mean of
// working-set reps is a RAW FRACTION (e.g. 8/7/7 -> 7.33) and is compared UNROUNDED
// against the range low/high in meetsTop()/belowBottom(). Rounding the fraction up would
// make 'mean' behave like 'best' and defeat the point of the owner's choice — never
// round it, in this file or downstream.
// Valid values: 'all' | 'first' | 'mean' | 'best'.
const TOP_OF_RANGE_RULE = 'mean' // owner decision 2026-09-09 (Дмитро, «роби поки mean»), #1291 comment #7422 — provisional, see comment above
